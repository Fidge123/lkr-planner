...
/// Rejects the write before it is issued when the event behind `href` belongs to a Daylite project marked as a fixed appointment.
/// The project link is read from the event itself, never from a caller-supplied reference, so the check cannot be bypassed from the frontend.
...
/// The committed part of a fixed appointment is its day, so a move that keeps the date is allowed even for a protected event.
...
/// An event without a Daylite reference was not created by the planner, so it is protected as well.
/// A project that cannot be resolved fails open, so a broken Daylite link never locks an event permanently.
