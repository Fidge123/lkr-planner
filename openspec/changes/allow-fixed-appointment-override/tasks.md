...
- [x] 3.1 Regenerate `src/generated/tauri.ts` by running the app in debug mode
- [x] 3.2 Update the other call sites for the changed signatures: the drag path (`use-appointment-drag.ts`) passes no override, the move-reconciliation dialog overrides so a half-finished move can always drop its duplicate

## 4. Modal unlock control

- [x] 4.1 Write failing `bun test`s: locked state disables picker, save and delete; unlocked state enables all three; reopening resets to locked
- [x] 4.2 Add the unlock state to `use-assignment-modal.ts`, reset it in the modal's open effect, and pass the override on both writes
- [x] 4.3 Render the unlock control inside the existing notice in `assignment-modal.tsx` and key the disabled conditions on "protected and not unlocked", satisfying the tests
