...
- [ ] 2a.2 (GREEN) Make `registerMock` generic over the generated `commands` from `src/generated/tauri.ts`, so the handler's return type must match the selected command's success payload (the raw value `invoke` resolves to, which `typedError` then wraps), not the wrapped `Result` return type. Derive it as `Extract<Awaited<ReturnType<typeof commands[K]>>, { status: "ok" }>["data"]`, turning shape mismatches into `tsc` (`bun`) failures. `// @ts-nocheck` on the generated file does not block this: its exported inferred types still flow to consumers
- [ ] 2a.2a (GREEN) Bridge the key mismatch: runtime dispatch keys on the snake_case command string (`"load_local_store"`) that the frontend passes to `invoke`, while the generated `commands` object keys on camelCase (`loadLocalStore`). The typed registry maps between the two so the snake_case registration resolves to the right command's payload type
...
- [ ] 2a.4 (GREEN) Create reusable typed fixture builders for the commands the smoke tests depend on (`daylite_list_contacts`, `load_local_store`, `load_week_events`, `get_holidays_for_week`), each returning the command's success payload typed against the generated bindings so drift surfaces in one place after regeneration

## 3. Baseline Smoke Tests

- [ ] 3.1 (RED) Create `tests/e2e/smoke.spec.ts` that navigates to `/`, registers the `invoke` mocks the planning view fires during initial render using the typed fixture builders from task 2a, and asserts a `data-testid`-selected element is visible with no unhandled JavaScript errors; it fails because the test ids do not exist yet. The required set is the four commands the startup effect fires: `daylite_list_contacts` (first, via `loadDayliteContacts`), `load_local_store`, `load_week_events`, and `get_holidays_for_week`. The latter two are reachable through callers that catch (`load_week_events`) or do not (`get_holidays_for_week` uses `.then` without `.catch`), and `load_local_store` is uncaught, so leaving any of them unregistered risks an unhandled rejection or a console error that trips the no-JS-errors assertion
