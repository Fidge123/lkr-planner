...
- Offer an unlock checkbox in the notice: ticking it re-enables the picker, save and delete for that modal session and sets an explicit override flag on the write, which the backend guard honours.
- If a write is attempted without the override (e.g. a stale UI state) and the backend rejects it, show the German error message returned by the backend.
