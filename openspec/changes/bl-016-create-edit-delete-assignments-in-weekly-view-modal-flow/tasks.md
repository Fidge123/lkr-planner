...
- [x] 2.9 Fix href resolution: use RFC-3986 `Url::join` against server origin instead of string-concatenating onto `zep_caldav_root_url` (prevented 404 on update/delete after grid reload)
...
- [x] 4.2 Write failing render test: modal in edit mode shows project picker and delete button
...
- [x] 4.6 Add `projectRef` to `CalendarCellEvent` and surface in TypeScript bindings so edit mode can pre-select the current project in the picker
