...
Let planners see where a project is already scheduled across the visible week without reading every card, by marking every card that carries the same Daylite project.
The highlight is switched on and off from a toggle on the card itself, so it stays up while the planner acts on what it shows.
