...
3. **Get apply instructions**
   ...
   **Handle states:**
   - If `state: "blocked"` (missing artifacts): show message, suggest using openspec-continue-change (if it is not installed, run `bunx openspec status --change "<name>" --json` to see the next artifact and `bunx openspec instructions <artifact-id> --change "<name>" --json` for how to create it)
